...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      ...
      const productAlreadyInCart = updateCart.find(product => product.id === productId);
  ...
  const updateProductAmount = async ({
    ...
    try {
      const updateCart = [...cart];
      const productIndexToUpdateAmount = updateCart.findIndex(product => product.id === productId);
